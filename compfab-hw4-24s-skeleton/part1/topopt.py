...
class TopologyOptimization:
    ...
    def initialize_boundary_conditions(self, bc_type: str):
        ...
        ## Cantilever beam
            ...
            # self.f[0, 0, 0] = 0
            self.f[grid_size_x, grid_size_y // 2, 1] = -1
    ...
    def optimality_criteria(self, s: array, fraction: float, change_limit: float):
        ...
        while l * (1 + 1e-15) < r:
            ...
            if np.sum(d_new) < fraction * np.size(d_new):        # <--
            # if true:
                # r = 0.0     # <--
                r = m
            else:
    ...
    def sensitivity_filtering(self, s: array, radius: float) -> array:
        ...
        # d_conv = d      # <--
        d_conv = convolve2d(s * d, w, mode = 'same')

        # Compute convolution for the numerator
        # --------
        # TODO: Your code here.
        # ds_conv = d     # <--
        ds_conv = convolve2d(d, w, mode = 'same')

        # Compute filtered sensitivities
        # --------
        # TODO: Your code here.
        # s_filtered = s      # <--
        s_filtered = d_conv / ds_conv
