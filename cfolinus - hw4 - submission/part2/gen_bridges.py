...
for radius in np.linspace(3.0, 4.0, 11):
    for offset in np.linspace(-3.0, -2.0, 11):
        ...
        # output mesh
        print(f'bridge_r_{int(radius * 10)}_o_{int(offset * 10)}.stl')
        mesh_file = os.path.join(meshes_dir, f'bridge_r_{int(radius * 10)}_o_{int(offset * 10)}.stl')
        subprocess.run(f"/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD -o {mesh_file} {scad_file}", shell=True)
