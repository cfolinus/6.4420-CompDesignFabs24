...
print(sys.path)
from voxelizer1 import Voxelizer
# import voxelizer
...
def set_boundary_conditions(mesh: TetMesh) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # f_ext[f_ext_mask] = 0                               # <--
    f_ext_mask = vertices[:,2] == bbox_max[2]
    f_ext[f_ext_mask] = np.array([0, 0, -5000])
    ...
    # To constrain the "feet" on the XZ plane
    # bc_mask = vertices[:,2] == bbox_min[2]
    
    # To constrain the left/right faces on the YZ planes...
    bc_mask = (vertices[:,0] == bbox_min[0]) | (vertices[:,0] == bbox_max[0])
...
def solve_performance(stl_file: str, save_tet_mesh: str='') -> Tuple[float, float]:
    ...
    # compliance = 0.0        # <--
    compliance = f_ext.ravel().T @ u.ravel()
    ...
    # values in an array.
    # mass = 0.0              # <--
    mass = np.count_nonzero(voxelizer.voxels)
